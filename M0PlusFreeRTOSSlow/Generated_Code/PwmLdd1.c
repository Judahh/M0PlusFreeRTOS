...
LDD_TDeviceData* PwmLdd1_Init(LDD_TUserData *UserDataPtr)
{
  ...
  DeviceDataPrv->LinkedDeviceDataPtr = TU0_Init((LDD_TUserData *)NULL);
...
LDD_TError PwmLdd1_Enable(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  if (!DeviceDataPrv->EnUser) {        /* Is the device disabled by user? */
    ...
    (void)TU0_Enable(DeviceDataPrv->LinkedDeviceDataPtr); /* Enable TimerUnit */
  }
  return ERR_OK;
}
...
static void SetRatio(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  (void)TU0_GetPeriodTicks(DeviceDataPrv->LinkedDeviceDataPtr, &Period);
  if (Period == 0U) {
  ...
  (void)TU0_SetOffsetTicks(DeviceDataPrv->LinkedDeviceDataPtr, CHANNEL, Duty);
}
/* END PwmLdd1. */
