 ...
 */
  ...
  #define configSTOPPED_TIMER_COMPENSATION    42UL  /* number of ticks to compensate, as defined in properties */
#endif /* configUSE_TICKLESS_IDLE */
...
__attribute__((weak)) void vPortSuppressTicksAndSleep(TickType_t xExpectedIdleTime) {
  ...
  if (eTaskConfirmSleepModeStatus()==eAbortSleep) {
    ...
    FreeRTOS0_vOnPreSleepProcessing(xExpectedIdleTime); /* go into low power mode. Re-enable interrupts as needed! */
    /* ----------------------------------------------------------------------------
     * Here the CPU *HAS TO BE* low power mode, waiting to wake up by an interrupt 
     * ----------------------------------------------------------------------------*/
    FreeRTOS0_vOnPostSleepProcessing(xExpectedIdleTime); /* process post-low power actions */
