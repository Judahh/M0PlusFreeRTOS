...
} SonarEcho0_TDeviceData;              /* Device data structure type */

typedef SonarEcho0_TDeviceData *SonarEcho0_TDeviceDataPtr ; /* Pointer to the device data structure. */

static const uint32_t SonarEcho0_PIN_MASK_MAP[2U] = {
   0x1000U, 0x2000U
};                                     /* Map of masks for each pin */

/* {FreeRTOS RTOS Adapter} Static object used for simulation of dynamic driver memory allocation */
static SonarEcho0_TDeviceData DeviceDataPrv__DEFAULT_RTOS_ALLOC;
...
LDD_TDeviceData* SonarEcho0_Init(LDD_TUserData *UserDataPtr)
{
  ...
  SonarEcho0_TDeviceDataPtr DeviceDataPrv;
  ...
  PE_LDD_RegisterDeviceStructure(PE_LDD_COMPONENT_SonarEcho0_ID,DeviceDataPrv);
...
uint32_t SonarEcho0_GetVal(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  PortData = GPIO_PDD_GetPortDataInput(SonarEcho0_MODULE_BASE_ADDRESS) & SonarEcho0_PORT_MASK;
  return PortData >> SonarEcho0_PIN_ALLOC_0_INDEX; /* Return port data shifted with the offset of the first allocated pin*/
}
...
LDD_TError SonarEcho0_GetBit(LDD_TDeviceData *DeviceDataPtr, uint8_t Bit, bool *BitVal)
{
  ...
  Mask = SonarEcho0_PIN_MASK_MAP[Bit];
  if ((GPIO_PDD_GetPortDataInput(SonarEcho0_MODULE_BASE_ADDRESS) & Mask) != 0U) {
    *BitVal = (bool)TRUE;
  } else {
    *BitVal = (bool)FALSE;
  }
