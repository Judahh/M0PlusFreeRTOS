...
*/         
/*!
**  @addtogroup UTIL0_module UTIL0 module documentation
**  @{
*/         
...
{
  UTIL0_Num16uToStr(dst, dstSize, (uint16_t)val);
}
...
{
  UTIL0_Num16sToStr(dst, dstSize, (int16_t)val);
}
...
void UTIL0_Num16sToStr(uint8_t *dst, size_t dstSize, int16_t val)
{
  ...
  if (val==(int16_t)(0x8000)) { /* special case 0x8000/-32768: prevent overflow below. */
    UTIL0_strcpy(dst, dstSize, (unsigned char*)"-32768");
    return;
  }
  dstSize--; /* for zero uint8_t */
  if (sign) {
...
{
  UTIL0_Num16sToStr(dst, dstSize, val);
...
{
  UTIL0_Num16uToStr(dst, dstSize, val);
  ShiftRightAndFill(dst, fill, nofFill);
}
...
{
  UTIL0_Num32uToStr(dst, dstSize, val);
  ShiftRightAndFill(dst, fill, nofFill);
}
...
{
  UTIL0_Num32sToStr(dst, dstSize, val);
  ShiftRightAndFill(dst, fill, nofFill);
}
...
void UTIL0_strcatNum8u(uint8_t *dst, size_t dstSize, uint8_t val)
{
  ...
  UTIL0_Num8uToStr(buf, sizeof(buf), val);
  UTIL0_strcat(dst, dstSize, buf);
}
...
void UTIL0_strcatNum8s(uint8_t *dst, size_t dstSize, signed char val)
{
  ...
  UTIL0_Num8sToStr(buf, sizeof(buf), val);
  UTIL0_strcat(dst, dstSize, buf);
}
...
void UTIL0_strcatNum16u(uint8_t *dst, size_t dstSize, uint16_t val)
{
  ...
  UTIL0_Num16uToStr(buf, sizeof(buf), val);
  UTIL0_strcat(dst, dstSize, buf);
}
...
void UTIL0_strcatNum16s(uint8_t *dst, size_t dstSize, int16_t val)
{
  ...
  UTIL0_Num16sToStr(buf, sizeof(buf), val);
  UTIL0_strcat(dst, dstSize, buf);
}
...
void UTIL0_strcatNum16uFormatted(uint8_t *dst, size_t dstSize, uint16_t val, char fill, uint8_t nofFill)
{
  ...
  UTIL0_Num16uToStrFormatted(buf, dstSize, val, fill, nofFill);
  UTIL0_strcat(dst, dstSize, buf);
}
...
void UTIL0_strcatNum16sFormatted(uint8_t *dst, size_t dstSize, int16_t val, char fill, uint8_t nofFill)
{
  ...
  UTIL0_Num16sToStrFormatted(buf, dstSize, val, fill, nofFill);
  UTIL0_strcat(dst, dstSize, buf);
}
...
void UTIL0_strcatNum32uFormatted(uint8_t *dst, size_t dstSize, uint32_t val, char fill, uint8_t nofFill)
{
  ...
  UTIL0_Num32uToStrFormatted(buf, dstSize, val, fill, nofFill);
  UTIL0_strcat(dst, dstSize, buf);
}
...
void UTIL0_strcatNum32sFormatted(uint8_t *dst, size_t dstSize, int32_t val, char fill, uint8_t nofFill)
{
  ...
  UTIL0_Num32sToStrFormatted(buf, dstSize, val, fill, nofFill);
  UTIL0_strcat(dst, dstSize, buf);
}
...
void UTIL0_strcatNum32s(uint8_t *dst, size_t dstSize, int32_t val)
{
  ...
  UTIL0_Num32sToStr(buf, sizeof(buf), val);
  UTIL0_strcat(dst, dstSize, buf);
}
...
void UTIL0_strcatNum32u(uint8_t *dst, size_t dstSize, uint32_t val)
{
  ...
  UTIL0_Num32uToStr(buf, sizeof(buf), val);
  UTIL0_strcat(dst, dstSize, buf);
}
...
void UTIL0_Num32sToStr(uint8_t *dst, size_t dstSize, int32_t val)
{
  ...
  if (val==(int32_t)(0x80000000)) { /* special case 0x80000000/-2147483648: prevent overflow below. */
    UTIL0_strcpy(dst, dstSize, (unsigned char*)"-2147483648");
    return;
  }
  dstSize--; /* for zero uint8_t */
  if (sign) {
...
uint8_t UTIL0_WeekDay(uint16_t year, uint8_t month, uint8_t day)
{
  ...
  sum += sum/4;
  sum %= 7;
  if (UTIL0_IsLeapYear(year) && (month==1 || month==2)) {
    sum--;
  }
  sum += day;
  sum %= 7;
...
uint8_t UTIL0_ScanDate(const unsigned char **str, uint8_t *day, uint8_t *month, uint16_t *year)
{
  ...
  if (   UTIL0_ScanDecimal8uNumber(&p, day)==ERR_OK
      && *day > 0 && *day <= 31
      && (*p=='.' || *p=='-')
     )
  {
    p++;
    if (   UTIL0_ScanDecimal8uNumber(&p, month)==ERR_OK
        && *month > 0 && *month <= 12
        && (*p=='.' || *p=='-')
       )
    {
      p++;
      if (   UTIL0_ScanDecimal16uNumber(&p, year)==ERR_OK
...
uint8_t UTIL0_ScanTime(const unsigned char **str, uint8_t *hour, uint8_t *minute, uint8_t *second, uint8_t *hSecond)
{
  ...
  *hSecond = 0;
  ...
  if (   UTIL0_ScanDecimal8uNumber(&p, hour)==ERR_OK
      && *hour <= 24
      && *p==':'
     )
  {
    p++; /* skip ':' */
    if (   UTIL0_ScanDecimal8uNumber(&p, minute)==ERR_OK
        && *minute <= 60
       )
    {
      if (*p==':') { /* there is more after the minute */
        p++; /* skip ':' */
        if (   UTIL0_ScanDecimal8uNumber(&p, second)==ERR_OK
            && *second <= 60
           )
        {
...
uint8_t UTIL0_ScanDecimal8sNumber(const unsigned char **str, signed char *val)
{
  ...
  *val = 0;
  ...
  res = UTIL0_ScanDecimal8uNumber(&p, &val8u);
  if (res != ERR_OK) {
    return res;
  }
  if (isNeg) {
...
uint8_t UTIL0_ScanDecimal16sNumber(const unsigned char **str, int16_t *val)
{
  ...
  *val = 0;
  ...
  res = UTIL0_ScanDecimal16uNumber(&p, (uint16_t*)&val16u);
  if (res != ERR_OK) {
    return res;
  }
  if (isNeg) {
...
byte UTIL0_ScanDecimal32sNumber(const unsigned char **str, int32_t *val)
{
  ...
  *val = 0;
  ...
  res = UTIL0_ScanDecimal32uNumber(&p, &val32u);
  if (res != ERR_OK) {
    return res;
  }
  if (isNeg) {
...
/*
** ===================================================================
**     Method      :  UTIL0_strcmp (component Utility)
**     Description :
...
*/
/***
int16_t UTIL0_strcmp(const char *, const char *)
{
...
/*
** ===================================================================
**     Method      :  UTIL0_strncmp (component Utility)
**     Description :
...
*/
/***
int16_t UTIL0_strncmp(const char *, const char *, size_t size)
{
...
/*
** ===================================================================
**     Method      :  UTIL0_strlen (component Utility)
**     Description :
...
*/
/***
uint16_t UTIL0_strlen(const char *)
{
...
uint8_t UTIL0_strtailcmp(uint8_t *str, uint8_t *tail)
{
  ...
  i = (int)UTIL0_strlen((char*)str);
  j = (int)UTIL0_strlen((char*)tail);
...
uint8_t UTIL0_strCutTail(uint8_t *str, uint8_t *tail)
{
  ...
  if (UTIL0_strtailcmp(str, tail)!=0) { /* check if tail is present */
    return ERR_FAILED; /* tail not found */
  }
  tailLen = UTIL0_strlen((char*)tail);
  strLen = UTIL0_strlen((char*)str);
...
{
  UTIL0_strcatNum32s(dst, dstSize, num/100);
  UTIL0_chcat(dst, dstSize, '.');
  if (num<0) {
    num = -num;
  }
  UTIL0_strcatNum16uFormatted(dst, dstSize, (uint16_t)(num%100), '0', 2);
}
...
int16_t UTIL0_strFind(uint8_t *str, uint8_t *subStr)
{
  ...
  len = (int16_t)UTIL0_strlen((char*)subStr);
  for (i=0; *str!='\0'; i++, str++) {
    if (UTIL0_strncmp((char*)str, (char*)subStr, len)==0) {
      return i; /* found */
    }
  }
  return -1; /* not found */
...
uint8_t UTIL0_ScanSeparatedNumbers(const unsigned char **str, uint8_t *values, uint8_t nofValues, char separator, UTIL0_SeparatedNumberType numberType)
{
  ...
  for(i=0;i<nofValues;i++) {
    if (numberType==UTIL0_SEP_NUM_TYPE_UINT8) {
      res = UTIL0_ScanDecimal8uNumber(&p, &values[i]);
    } else if (numberType==UTIL0_SEP_NUM_TYPE_UINT8_HEX_NO_PREFIX) {
      res = UTIL0_ScanHex8uNumberNoPrefix(&p, &values[i]);
    } else {
      res = ERR_FAILED;
    }
